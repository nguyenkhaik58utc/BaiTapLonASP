...
// ti`m kiê´m
temp = 0;
function functionSearch(searchId, index) {
	$(".addrow").removeAttr("disabled");
...
function PagitrationByMonth(id, index) {
	...
	temp;
	arrayData = [];
	if (month == 1 && id == 'PreviousByMonth1') {
		year = year - 1;
		month = 12;
	} else if (month == 12 && id == 'NextByMonth1') {
		year = year + 1;
		month = 1;
	}
	else {
	...
	$
		.ajax({
			...
			success: function (res) {
				var data = "";
				for (var i = 0; i < res.length; i++) {
					arrayData[i] = { "otId": res[i].otId, "registrationDate": res[i].registrationDate, "timeStart": res[i].timeStart, "timeFinish": res[i].timeFinish, "reason": res[i].reason };
					// arrAddr[i] =
				}
				for (var i = 0; i < res.length; i++) {
